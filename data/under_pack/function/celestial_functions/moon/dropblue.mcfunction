...
execute at @e[type=minecraft:snowball,nbt={Item:{components:{"minecraft:custom_data":{celedrop:1b,blue:1b}}}}] positioned ~ ~-1 ~ as @a[team=uRed,distance=..2.2] run damage @s 3.5 player_attack by @a[team=uBlue,scores={class=20},limit=1]
#kill after hit
execute as @e[type=minecraft:snowball,nbt={Item:{components:{"minecraft:custom_data":{celedrop:1b,blue:1b}}}}] at @s positioned ~ ~-1 ~ if entity @a[team=uRed,distance=..2.2] run kill @s
...
execute at @e[type=minecraft:snowball,nbt={Item:{components:{"minecraft:custom_data":{celedrop:1b,blue:1b}}}}] if entity @s[dx=1] run scoreboard players set @s ability3 21