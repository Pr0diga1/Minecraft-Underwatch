...
execute if score @s ability1 matches 10.. run playsound minecraft:block.note_block.banjo master @p[team=uRed] ~ ~ ~ 1 0.5

execute positioned ~-0.5 ~-0.5 ~-0.5 as @a[dx=1,dy=1,dz=1,team=uRed] run return run function under_pack:bard_barian/note_damage
execute facing entity @p[team=uRed] eyes run tp ^ ^ ^0.01