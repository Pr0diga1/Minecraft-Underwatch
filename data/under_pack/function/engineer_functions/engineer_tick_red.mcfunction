#magnet actionbar
execute at @s if entity @a[team=uBlue,scores={engineermagtimer=2.., ability4=0}] run title @s actionbar [{"text":""},{"text":"Magnetization fading: ","color":"white"},{"score":{"name":"@a[team=uBlue,scores={engineermagtimer=2..},limit=1,sort=nearest]","objective":"engineermagtimer"},"bold":true,"color":"red"}]
execute at @s if entity @a[team=uBlue,scores={engineermagtimer=1, ability4=0}] run title @s actionbar {"text":"No enemies magnetized","color":"white"}
...
#wall delete blocks
execute at @s as @e[type=falling_block,tag=red,tag=enginewall,dx=-0.8,dy=0.5,dz=-0.8] run kill @s