...
execute as @a[tag=lake] run function under_pack:general_functions/general_classloop
...
execute as @a[tag=lake,tag=uDead] run scoreboard players add @s deathTimer 1

#creates a buffer that stores seconds instead of TickToSecond
#buffer = deathTimer
execute as @a[tag=lake,tag=uDead] run scoreboard players operation @s deathTimerBuffer = @s deathTimer
#buffer converted from ticks to seconds
execute as @a[tag=lake,tag=uDead] run scoreboard players operation @s deathTimerBuffer /= TickToSecond constant
#second buffer = first buffer
execute as @a[tag=lake,tag=uDead] run scoreboard players operation @s deathTimerBufferBuffer = @s deathTimerBuffer 
#first buffer is now 10
execute as @a[tag=lake,tag=uDead] run scoreboard players operation @s deathTimerBuffer = DeathBuffer constant
# 10 - the number of seconds that have past, gives you number of seconds left
execute as @a[tag=lake,tag=uDead] run scoreboard players operation @s deathTimerBuffer -= @s deathTimerBufferBuffer
#actionbar for telling players when they will respawn
execute as @a[tag=lake,tag=uDead] run title @s actionbar ["",{"text":"Respawn in: ","color":"gold"},{"score":{"name":"@s","objective":"deathTimerBuffer"},"color":"red"},{"text":" seconds","color":"gold"}]

#resets players when they have been dead for 10 seconds
#sets them to adventure
execute as @a[tag=lake,tag=uDead] if score @s deathTimer matches 300.. run gamemode adventure @s
#tp them to their spanws
execute as @a[tag=lake,tag=uDead,team=uRed] if score @s deathTimer matches 300.. run tp @s -497 86 -575
execute as @a[tag=lake,tag=uDead,team=uBlue] if score @s deathTimer matches 300.. run tp @s -508 86 -457
#tells them they respawned 
execute as @a[tag=lake,tag=uDead] if score @s deathTimer matches 300.. run title @s actionbar {"text":"Respawned","color":"gold"}
#run the general respawn
execute as @a[tag=lake,tag=uDead] if score @s deathTimer matches 300.. run function under_pack:general_functions/general_respawn
#remove dead tag
execute as @a[tag=lake,tag=uDead] if score @s deathTimer matches 300.. run tag @s remove uDead
