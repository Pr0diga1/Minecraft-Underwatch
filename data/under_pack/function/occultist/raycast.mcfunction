...
#rerun command
execute if entity @s[team=uBlue] unless entity @a[tag=target,team=uBlue] if score distance buffer matches ..180 positioned ^ ^ ^.5 run function under_pack:occultist/raycast
execute if entity @s[team=uRed] unless entity @a[tag=target,team=uRed] if score distance buffer matches ..180 positioned ^ ^ ^.5 run function under_pack:occultist/raycast
