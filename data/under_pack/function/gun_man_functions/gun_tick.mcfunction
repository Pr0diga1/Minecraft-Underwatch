...
execute if score @s guncoinbuffer matches 1.. run scoreboard players remove @s guncoinbuffer 1
...
#fire the coin
execute if score @s guncoinbuffer matches 0 at @s if entity @e[sort=nearest,limit=1,distance=..2,type=item,nbt={Item:{components:{"minecraft:custom_data":{guncoin:true}}}}] run function under_pack:gun_man_functions/coin_lob
