...
execute at @s[scores={hitByFreeze=30}] run summon marker ~ ~ ~ {Tags:["sciFreeze","red"]}

#players who are frozen get tpd
execute as @s[scores={hitByFreeze=1..}] at @e[type=marker,tag=sciFreeze,tag=red,limit=1,sort=nearest] run tp @s ~ ~ ~
