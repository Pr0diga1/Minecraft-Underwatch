...
execute if score @s gun_ar_cd matches 1 if entity @s[nbt={Inventory:[{Slot:0b,tag:{gunar:1b}}],SelectedItem:{id:"minecraft:ender_eye",tag:{gunar:1b}}},scores={reset=1..}] as @s run function under_pack:gun_man_functions/gun_ar