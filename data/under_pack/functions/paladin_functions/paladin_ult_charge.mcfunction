...
scoreboard players operation @s ultKillsBuffer *= paladinUltKills modifier
...
scoreboard players operation @s ultPercent /= paladinUltPercent modifier
...
#updates levels
function under_pack:paladin_functions/paladin_ult_update

#resets ultPercent so the next buffer is accurate
scoreboard players operation @s ultPercent = @s ultPercentBuffer

#give the carrot on a stick if the ult charge is 100%
execute unless entity @s[nbt={Inventory:[{Slot:8b,tag:{paladinUlt:1b}}]}] if score @s ultPercentBuffer matches 100.. run item replace entity @s hotbar.8 with carrot_on_a_stick{display:{Name:'{"text":"Unga Bunga","color":"black"}'},paladinUlt:1b} 1