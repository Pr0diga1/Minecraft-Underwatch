...
scoreboard players operation @s ultKillsBuffer *= archerUltKills modifier
...
scoreboard players operation @s ultPercent /= archerUltPercent modifier
...
#updates levels
function under_pack:archer_functions/archer_update_ult

#resets ultPercent so the next buffer is accurate
scoreboard players operation @s ultPercent = @s ultPercentBuffer

#give the carrot on a stick if the ult charge is 100%
execute unless entity @s[nbt={Inventory:[{Slot:8b,tag:{archerult:1b}}]}] if score @s ultPercentBuffer matches 100.. run item replace entity @s hotbar.8 with carrot_on_a_stick{display:{Name:'{"text":"Machine Gun","color":"gold"}'},archerult:1b} 1